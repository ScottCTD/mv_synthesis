...
def render_clips_parallel(plans: list[PostprocessPlan], workers: int) -> None:
    if not plans:
        return
    max_workers = max(1, workers)
    if max_workers == 1:
        for plan in plans:
            render_postprocess_plan(plan)
        return
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(render_postprocess_plan, plan) for plan in plans]
